...
AGENTS = [
    ...
    },
    {
        ...
        "system_prompt": (
            ...
            "- Темы для контента (маркетинг): по запросам «темы для постов», «что писать», "
            "«дайджест каналов» вызывай content_digest — он читает отслеживаемые ТГ-каналы и "
            "выделяет темы/тренды. Список каналов веди через add_content_channel / "
            "list_content_channels / remove_content_channel.\n"
            "- Мониторинг запчастей (закупки): по запросам «следи за ценами на …», «добавь в "
            "мониторинг запчастей» веди список через add_part_query / list_part_queries / "
            "remove_part_query. Сбор объявлений с Авито/Дром идёт фоном по расписанию (уважая "
            "robots.txt площадок); копится история цен и дат снятия для аналитики спроса.\n"
            "- Контент-план (маркетинг): по запросам «контент-план», «план публикаций», «что "
            "постить» СНАЧАЛА подними линейку, акции и сезонность через search_knowledge_base, "
            "затем вызови create_content_plan (Word/Excel). Не выдумывай характеристики и акции: "
            "чего нет в БЗ — в gaps «уточнить: …», в seasonality объясни выбор тем и сроков.\n"
            "- Анализ рынка и конкурентов: по запросам «проанализируй рынок», «что у конкурентов», "
            "«цены на рынке» СНАЧАЛА собери фактуру веб-поиском и read_url, затем вызови "
            "analyze_market (отчёт в Word/Excel). Только по найденному: чего нет в источниках — "
            "перечисли в gaps, не додумывай цифры; у позиций указывай ссылку-источник.\n"
