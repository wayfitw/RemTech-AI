...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "По умолчанию — КЛАССИЧЕСКИЙ стиль (как в шаблонах базы знаний, без бланка). Фирменный "
            "макет «Ремтехники» (бланк RT + реквизиты) применяй ТОЛЬКО если пользователь явно "
            "попросил оформить в стиле «Ремтехники» — тогда style=remtekhnika."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "style": {"type": "string", "enum": ["remtekhnika", "classic"],
                          "description": "classic — без бланка (ПО УМОЛЧАНИЮ, как шаблоны БЗ); remtekhnika — фирменный бланк, ТОЛЬКО если пользователь попросил."},
            },
    ...
    },
    {
        ...
        "description": (
            ...
            "По умолчанию — КЛАССИЧЕСКИЙ стиль (как в шаблонах БЗ, без бланка). Фирменный макет "
            "«Ремтехники» (с бланком) применяй ТОЛЬКО если пользователь явно попросил — style=remtekhnika. "
            "Доступ: продажи/руководство."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
