...
class Settings(BaseSettings):
    ...
    # Папка с реальными фото техники/объектов для слайдов (гибрид: реальные фото
    # приоритетны, AI-генерация — для обложки/разделов/концепций). Имя файла = ключ
    # поиска (напр. «xcmg-xe215.jpg», «liugong 856h.png»). Пусто → только AI.
    presentation_assets_dir: str = "data/assets/equipment"
    # Генерировать AI-картинки (FLUX) для слайдов, где нет реального фото. false →
    # презентации только с реальными фото (и без картинок там, где фото не нашлось).
    presentation_ai_images: bool = True
    # Предел одновременных генераций картинок на одну презентацию (Replicate rate).
    presentation_image_concurrency: int = 4
