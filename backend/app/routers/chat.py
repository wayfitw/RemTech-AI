...
async def api_conversations(user: dict = Depends(current_user),
    ...
    # Только web-диалоги: истории из Telegram-бота в веб не показываем (тг-тг, веб-веб).
    return [conv_dict(c) for c in await repo.list_conversations(db, user["user_id"], channel="web")]
...
async def api_delete_conversation(conversation_id: int, user: dict = Depends(current_user),
    ...
    if not conv or conv.user_id != user["user_id"] or conv.channel != "web":
